...
| Couche | On-premise | IaaS (ex: EC2) | PaaS (ex: Elastic Beanstalk) | SaaS (ex: WorkMail) |
|---|---|---|---|---|
...
- Tu as besoin d'un outil métier standard (email, collaboration, CRM) sans développement → **SaaS**
...
content: IaaS → EC2, EBS, VPC. PaaS → Elastic Beanstalk, RDS, Lambda (pour la partie exécution). SaaS → WorkMail, Chime, Amazon Connect. La frontière PaaS/SaaS est parfois floue selon le niveau de configuration disponible.
...
AWS n'est pas un datacenter unique quelque part aux États-Unis. C'est un réseau mondial d'infrastructure physique organisé en trois niveaux imbriqués.

### Les régions

Une **région AWS** est un ensemble de datacenters physiquement localisés dans une même zone géographique. Chaque région est complètement indépendante des autres — elle a son propre réseau, ses propres services, et ses propres données ne sortent jamais de cette région sans ton autorisation explicite.

Exemples : `us-east-1` (Virginie du Nord), `eu-west-3` (Paris), `ap-southeast-1` (Singapour).

Pourquoi c'est important : la région que tu choisis détermine la latence pour tes utilisateurs, le cadre légal applicable à tes données (RGPD en Europe), et la disponibilité de certains services AWS.

### Les zones de disponibilité (AZ)

Chaque région contient entre 2 et 6 **zones de disponibilité** (Availability Zones). Une AZ, c'est un ou plusieurs datacenters physiquement séparés, avec leur propre alimentation électrique, réseau et système de refroidissement.
...
En code, les AZ s'appellent `us-east-1a`, `us-east-1b`, `us-east-1c`, etc.

### Les edge locations

Les **edge locations** ne sont pas des AZ. Ce sont des points de présence (PoP) déployés dans des dizaines de villes dans le monde, utilisés principalement par **CloudFront** (le CDN d'AWS) et Route 53 (DNS). Leur rôle : mettre en cache du contenu au plus près de tes utilisateurs finaux pour réduire la latence.
...
Le principe : **AWS et toi partagez la responsabilité de la sécurité**, mais pas sur les mêmes périmètres.
...
AWS gère et sécurise l'infrastructure physique : les datacenters (accès physique, caméras, badges), les hyperviseurs, le réseau physique entre ses installations, le matériel de calcul, stockage et réseau. Tu n'as aucun accès à cette couche — et c'est voulu.

### Ce qui reste sous ta responsabilité

Dès que tu lances une ressource, la responsabilité se déplace vers toi :

- **Sur EC2 (IaaS)** : l'OS et ses mises à jour, la configuration du pare-feu (Security Groups), les accès SSH, les données stockées sur le volume EBS, et la configuration IAM des utilisateurs qui accèdent à l'instance.
...
### Responsabilité selon le modèle de service
...
title: Responsabilité partagée — les erreurs fréquentes
...
Ces deux termes apparaissent partout dans la documentation AWS. Voici ce qu'ils signifient concrètement.
...
🧠 Une architecture haute disponibilité n'est pas forcément scalable, et vice versa. Une app déployée sur deux instances fixes (multi-AZ) est hautement disponible, mais ne scale pas automatiquement. Combine les deux pour une architecture production-ready.
...
content: Scalabilité = capacité à absorber la charge (scale up = instance plus grosse, scale out = plus d'instances). Haute disponibilité = résilience aux pannes via redondance multi-AZ. Les deux sont complémentaires mais indépendants. Une app multi-AZ sur deux instances fixes est HA mais pas scalable.
...
**Problème** : impossible de scale verticalement sans coupure, et louer un second serveur en permanence doublerait les coûts pour un usage épisodique.

**Choix AWS** : après analyse, l'équipe opte pour EC2 (IaaS) plutôt qu'Elastic Beanstalk (PaaS) — leur stack PHP/Laravel utilise des extensions personnalisées qui nécessitent un contrôle fin sur l'OS. Ils déploient sur la région `eu-west-3` (Paris) pour la latence et le RGPD.

**Architecture mise en place** :
- 2 instances EC2 `t3.medium` en Auto Scaling Group sur 2 AZ (`eu-west-3a` et `eu-west-3b`)
...
**1. Toujours choisir la région en premier, avant de créer quoi que ce soit**  
...
**2. Déployer systématiquement sur au moins deux AZ**  
...
**5. Anticiper la gouvernance des coûts dès le départ**  
Le modèle pay-as-you-go est un avantage, mais aussi un piège si tu ne surveilles pas ta consommation. Activer le billing alerts et les budgets AWS dès la création du compte — avant de lancer la moindre ressource.

**6. Toujours tagger ses ressources**  
...
**7. Comprendre le modèle de responsabilité avant de configurer la sécurité**  
Avant de toucher aux Security Groups ou aux politiques IAM, avoir en tête ce qu'AWS gère et ce que tu dois gérer toi-même. La majorité des incidents de sécurité sur AWS sont dus à des mauvaises configurations côté client.
...
Le modèle de responsabilité partagée est la règle d'or : AWS sécurise l'infrastructure, toi tu sécurises ce que tu construis dessus. Cette distinction change radicalement selon que tu utilises EC2 (IaaS) ou RDS (PaaS).

Les modules suivants s'appuient directement sur ces fondations : IAM formalise la gestion des identités et des permissions (ta part de la responsabilité partagée), EC2 est ton premier contact concret avec l'IaaS AWS, et le VPC te donnera le contrôle sur le réseau dans lequel tout ça s'exécute.