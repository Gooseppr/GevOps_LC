...
> - "transient errors / erreurs transitoires" + "retry logic / logique de réessai" → **Exponential Backoff with Jitter** (évite le thundering herd)
> - "circuit breaker" + "prevent cascade failure / empêcher les pannes en cascade" → pattern **Circuit Breaker** (Closed → Open → Half-Open)
> - "graceful degradation / dégradation gracieuse" → réponse de secours (cache, valeur par défaut) quand un service est down
> - "test resilience / tester la résilience" + "inject failures / injecter des pannes" + "chaos engineering" → **AWS FIS** (Fault Injection Simulator)
> - "idempotent / idempotent" → une opération qui peut être exécutée plusieurs fois avec le même résultat. Essentiel avec SQS (at-least-once delivery)
> - "SQS message processed twice / message SQS traité deux fois" → rendre le consumer **idempotent** ou passer en **SQS FIFO** (exactly-once)
> - ⛔ Retry **uniquement** sur les erreurs transitoires (5xx, throttling). Ne **jamais** retry sur les erreurs 4xx (bad request, unauthorized).
> - ⛔ "Thundering herd" = tous les clients retry en même temps → **jitter** (aléa) résout ce problème.
