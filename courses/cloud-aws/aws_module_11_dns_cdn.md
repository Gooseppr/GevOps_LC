...
> **SAA-C03** — "zone apex" (example.com) → **Alias A record** (jamais CNAME). "update files behind CloudFront" → **versioned objects** (pas invalidation). CloudFront = **cache CDN**. Global Accelerator = **routing réseau** (static IP, pas de cache).
